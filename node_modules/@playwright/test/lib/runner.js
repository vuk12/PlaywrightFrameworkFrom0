...
class Runner {
  ...
  async _createReporter(list) {
    ...
    if (reporters.length && !someReporterPrintsToStdio) {
      ...
      reporters.unshift(process.stdout.isTTY && !process.env.CI ? new _line.default({
        omitFailures: true
      }) : new _dot.default({
        omitFailures: true
      }));
  ...
  async _run(list, testFileReFilters, projectNames) {
    ...
    if (config.globalSetup && !list) globalSetupResult = await (await this._loader.loadGlobalHook(config.globalSetup, 'globalSetup'))(this._loader.fullConfig());
    ...
    try {
      ...
      config.__testGroupsCount = testGroups.length;
      let sigint = false;
      let sigintCallback;
      ...
      if (config.globalTeardown && !list) await (await this._loader.loadGlobalHook(config.globalTeardown, 'globalTeardown'))(this._loader.fullConfig());
