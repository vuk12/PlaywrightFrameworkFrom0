...
function toMatchSnapshot(received, nameOrOptions, optOptions = {}) {
  ...
  if (Array.isArray(nameOrOptions) || typeof nameOrOptions === 'string') options = {
    name: nameOrOptions,
    ...optOptions
  ...
  if (options.threshold === undefined && projectThreshold !== undefined) options.threshold = projectThreshold; // sanitizes path if string

  const pathSegments = Array.isArray(options.name) ? options.name : [(0, _util.addSuffixToFilePath)(options.name, '', undefined, true)];
  ...
  const {
    ...
    diffPath,
    mimeType
  } = (0, _golden.compare)(received, pathSegments, testInfo.snapshotPath, testInfo.outputPath, updateSnapshots, withNegateComparison, options);
