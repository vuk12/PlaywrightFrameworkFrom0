...
async function createLoader(opts) {
  ...
  if (opts.debug) process.env.PWDEBUG = '1';
  const loader = new _loader.Loader(defaultConfig, overrides);
  ...
  async function loadConfigFromDirectory(directory) {
    ...
    for (const configName of configNames) {
      if (await loadConfig(_path.default.resolve(directory, configName))) return true;
    }
  ...
  if (opts.config) {
    const configFile = _path.default.resolve(process.cwd(), opts.config);

    if (!_fs.default.existsSync(configFile)) throw new Error(`${opts.config} does not exist`);

    if (_fs.default.statSync(configFile).isDirectory()) {
...
async function runTests(args, opts) {
  ...
  const runner = new _runner.Runner(loader);
